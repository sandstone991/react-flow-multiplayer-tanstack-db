...
import { $acceptInvite, $createWorkspace, $declineInvite } from "@/lib/workspaces/functions";
import {
...
export const Route = createFileRoute("/_auth/join")({
  validateSearch: (search: Record<string, unknown>) => {
    if (search.manage === "1" || search.manage === "true" || search.manage === true) {
      return { manage: true };
    }
...
function JoinPage() {
  ...
  const [pendingAcceptInviteId, setPendingAcceptInviteId] = useState<string | null>(null);
  ...
  const { mutate: createWorkspaceMutate, isPending: isCreateWorkspacePending } = useMutation({
    mutationFn: async () =>
      $createWorkspace({
        data: {
          name: workspaceName,
    ...
    onSuccess: async (createdWorkspace) => {
      setWorkspaceName("");
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: joinDataQueryOptions().queryKey }),
        queryClient.invalidateQueries({ queryKey: myWorkspacesQueryOptions().queryKey }),
        queryClient.invalidateQueries({ queryKey: ["workspaces", "post-login-redirect"] }),
      ]);
      await navigate({
        to: "/app",
        search: {
  ...
  const { mutate: declineInviteMutate } = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: joinDataQueryOptions().queryKey });
    },
    onError: (error) => {
    ...
    <div className="mx-auto flex min-h-svh w-full max-w-3xl flex-col gap-6 p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Create a workspace or accept an invitation before opening shared todos.
          </p>
        </div>
        <SignOutButton />
      </div>
      ...
      <section className="rounded-md border p-4">
        ...
        >
          ...
          <Button type="submit" disabled={!hasWorkspaceName || isCreateWorkspacePending}>
            {isCreateWorkspacePending && <RiLoader4Line className="animate-spin" />}
      ...
      <section className="rounded-md border p-4">
        <div className="mb-3 flex items-center justify-between gap-2">
          ...
          {joinData.joinedWorkspaces[0] ? (
            <Button
              variant="outline"
              size="sm"
              render={<Link to="/app" search={{ workspaceId: joinData.joinedWorkspaces[0].id }} />}
              nativeButton={false}
            >
              Open app
            </Button>
        ...
        {joinData.joinedWorkspaces.length === 0 ? (
          <p className="text-sm text-muted-foreground">You have not joined any workspace yet.</p>
        ) : (
            ...
            {joinData.joinedWorkspaces.map((joinedWorkspace) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Role: {joinedWorkspace.role}</p>
                </div>
                <Button
                  size="sm"
                  variant="outline"
                  render={<Link to="/app" search={{ workspaceId: joinedWorkspace.id }} />}
                  nativeButton={false}
                >
                  Open
                </Button>
        ...
        {joinData.invitedWorkspaces.length === 0 ? (
          ...
          <ul className="space-y-2">
            {joinData.invitedWorkspaces.map((invitedWorkspace) => {
              const isAccepting = pendingAcceptInviteId === invitedWorkspace.inviteId;
              const isDeclining = pendingDeclineInviteId === invitedWorkspace.inviteId;
              ...
              return (
                ...
                >
                  <div>
                    <p className="font-medium">{invitedWorkspace.workspaceName}</p>
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      {isAccepting && <RiLoader4Line className="animate-spin" />}
                      Accept
                    </Button>
                    <Button
                      size="sm"
                    ...
                    >
                      {isDeclining && <RiLoader4Line className="animate-spin" />}
                      Decline
                    </Button>
                  </div>
                </li>
