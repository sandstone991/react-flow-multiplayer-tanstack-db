...
const resolveWorkspaceRedirect = async (userId: string) => {
	...
	const validLastWorkspaceId =
		...
		membershipWorkspaceIds.has(dbUser.lastWorkspaceId)
			? dbUser.lastWorkspaceId
			: null;

	const fallbackWorkspaceId =
		validLastWorkspaceId ?? memberships[0]?.workspaceId ?? null;
...
export const $getWorkspaceMembers = createServerFn({ method: "GET" })
	.middleware([authMiddleware])
	.inputValidator((data) => workspaceIdInput.parse(data))
	.handler(async ({ context, data }) => {
		...
		const pendingInvites = await db
			...
			.from(workspaceInvite)
			.innerJoin(user, eq(workspaceInvite.invitedById, user.id))
			.where(
				and(
					eq(workspaceInvite.workspaceId, data.workspaceId),
					eq(workspaceInvite.status, "pending"),
				),
			)
			.orderBy(desc(workspaceInvite.createdAt));

		return { members, pendingInvites, currentUserRole: membership.role };
	});
...
export const $sendInvite = createServerFn({ method: "POST" })
	.middleware([freshAuthMiddleware])
	.inputValidator((data) => sendInviteInput.parse(data))
	.handler(async ({ context, data }) => {
		const inviteEmail = normalizeEmail(data.email);

		if (inviteEmail === normalizeEmail(context.user.email)) {
...
export const $declineInvite = createServerFn({ method: "POST" })
	...
	.handler(async ({ context, data }) => {
		...
		if (normalizeEmail(invite.email) !== normalizeEmail(context.user.email)) {
			throw new Error(
				"You can only decline invites sent to your email address.",
			);
		}
